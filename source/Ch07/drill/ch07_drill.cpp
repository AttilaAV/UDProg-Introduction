...
//function declarations because they are after main()
double expression();
double primary();
double term();
double declaration();
...
{
	// checking if the name is already in the vector
...
{
	//here we are adding it to the vector
...
Token Token_stream::get()
{
	...
	switch (ch)
	{
    	...
    	{
    		//we need to put it back otherwise larger numbers wouldnt work
    		cin.putback (ch);
    		double val;
    		cin >> val;
...
void calculate()
{
	while (cin)
	try {
		//we are getting out input(token)
		//checking if its 'quit' or a statement
		Token t = ts.get();
...
double calc_sqrt(){
	...
	cin.putback(ch);
	double d = expression(); // between () we will have an expression
	if (d < 0) error("sqrt can't handle negative values");
	return sqrt(d); // from header math
...
double statement()
{
	//getting our token and checking if it will be a declaration or an expression
	Token t = ts.get();
	switch(t.kind)
	{
		case let:
